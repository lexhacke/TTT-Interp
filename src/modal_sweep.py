...
def run_single(
    ...
    logger = WandbLogger(
        project="ttt-interp",
        name=f"{backbone}_h{hidden}_rho{rho}_kv{num_kv_pairs}{ow_tag}",
        config={
...
def main():
    ...
    for backbone, hidden, rho in itertools.product(backbones, hidden_sizes, rhos):
        ...
        runs.append((backbone, hidden, rho, ttt_hidden_mult, heads))

    print(f"Launching {len(runs)} runs on Modal (kv={num_kv_pairs}, ow={num_overwrites})...")

    handles = []
    for backbone, hidden, rho, mult, h in runs:
